...
describe('parseAudienceCsv', () => {
  ...
  it('erro carrega código estável para a UI traduzir', () => {
    try {
      ...
      expect((err as SpreadsheetParseError).code).toBe('missing_phone_column');
    }
  });
...
// contra entradas reais antes de escrever a instrução de formatação.
describe('parseAudienceCsv — casos A-K da SPEC 052', () => {
  it('A: CSV `,` com tags final `vip,lead` sem aspas perde a segunda etiqueta', () => {
    // Documenta a perda: sem aspas, a vírgula da célula é lida como
    // separador de coluna. A nota da §6.2 é a defesa (usar `;`).
    const csv = 'phone,name,tags\n5511988887777,Maria,vip,lead';
    const rows = parseAudienceCsv(csv);

    expect(rows[0].tagNames).toEqual(['vip']);
  });

  it('B: CSV `,` com tags `"vip, lead"` entre aspas preserva as duas', () => {
    const csv = 'phone,name,tags\n5511988887777,Maria,"vip, lead"';
    const rows = parseAudienceCsv(csv);
  ...
  it('C: CSV `,` com tags `vip;lead` sem aspas funciona sem aspas', () => {
    const csv = 'phone,name,tags\n5511988887777,Maria,vip;lead';
    const rows = parseAudienceCsv(csv);
  ...
  it('D: CSV `,` com tags no meio sem aspas desloca as colunas seguintes', () => {
    // Comportamento conhecido, não corrigido nesta fase — travado por
    // teste para que uma mudança futura no tokenizador não o altere
    // por acidente sem que alguém perceba.
    const csv = 'phone,tags,name\n5511988887777,vip,lead,Maria';
    const rows = parseAudienceCsv(csv);
  ...
  it('E: CSV `;` (padrão do Excel em pt-BR) reconhece as 3 colunas', () => {
    const csv = 'phone;name;email\n5511988887777;Maria;maria@exemplo.com';
    const rows = parseAudienceCsv(csv);
  ...
  it('E2: CSV `;` com tags `"vip, lead"` entre aspas — aspas valem nos dois delimitadores', () => {
    const csv = 'phone;name;tags\n5511988887777;Maria;"vip, lead"';
    const rows = parseAudienceCsv(csv);
  ...
  it('E3: CSV `,` com vírgula dentro de campo citado não muda o delimitador farejado', () => {
    const csv = 'phone,name,tags\n5511988887777,"Silva, Maria","vip, sp"';
    const rows = parseAudienceCsv(csv);
  ...
  it("F: preserva apóstrofo em nomes — Maria D'Ávila, O'Brien & Co", () => {
    const csv = "phone,name,company\n5511988887777,Maria D'Ávila,O'Brien & Co";
    const rows = parseAudienceCsv(csv);
  ...
  it('G: desescapa aspas duplicadas entre aspas (RFC 4180)', () => {
    const csv = 'phone,name\n5511988887777,"Maria ""Bibi"" Souza"';
    const rows = parseAudienceCsv(csv);

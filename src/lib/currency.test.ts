...
/**
 * Digits-only view of a formatted currency string. Intl grouping
 * separators vary by locale (`1,234` en-US vs `1.234` pt-BR), so
 * assertions must not hardcode a single grouping character.
 */
...
describe('formatCurrency', () => {
  it('formats whole amounts with no minor units', () => {
    ...
    // Whole-number mode: no trailing ".00" / ",00" fraction pair.
  ...
  it('never throws on a structurally invalid code (no DB CHECK on deals.currency)', () => {
    for (const bad of ['United States', 'US', 'USDD', '12', 'u$d']) {
      ...
      // Amount digits must appear; the code itself may also contain digits
      // (e.g. "12" → "12 1.234"), so assert a trailing 1234 rather than equality.
      expect(digitsOnly(formatCurrency(1234, bad))).toMatch(/1234$/);
    }
  });

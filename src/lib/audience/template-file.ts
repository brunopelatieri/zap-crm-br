 ...
 *
 * Gerado a partir de `AUDIENCE_COLUMNS` em vez de ser um arquivo
...
import { AUDIENCE_COLUMNS } from './parse-csv';

export const AUDIENCE_TEMPLATE_FILENAME = 'modelo-audiencia-zap-crm.csv';

/** Uma linha por coluna de `AUDIENCE_COLUMNS`, na mesma ordem. */
...
export function buildAudienceTemplateCsv(): string {
  const header = AUDIENCE_COLUMNS.map((column) => column.aliases[0]);

        ...
        <section>
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
            {TEMPLATE_ORDER.map((slug) => {
              ...
              const nameKey =
                `templateCards.${slug}.name` as `templateCards.${TemplateSlug}.name`
              const descKey =
                `templateCards.${slug}.description` as `templateCards.${TemplateSlug}.description`
              return (
                <button
                  key={slug}
                ...
                >
                  ...
                  <div className="text-sm font-semibold text-foreground">
                    {t.has(nameKey) ? t(nameKey) : tpl.name}
                  </div>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {t.has(descKey) ? t(descKey) : tpl.description}
                  </p>
                </button>
              )
            })}
...
function AutomationCard({
  ...
  const triggerLabel = tTriggers.has(`${triggerType}.label`)
    ? tTriggers(`${triggerType}.label` as "new_message_received.label")
    : meta.label
  return (

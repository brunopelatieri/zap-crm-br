...
-- conversations.channel_id entra na 059 (renumerada de 057 — ver SPEC 048 §3), já com o backfill possível.
```
...
### Migração 059 — conversas por canal

> ⚠️ Renumerada de 057 para 059 em 2026-08-14 — 057 e 058 foram consumidos por commits
> não-relacionados antes desta rodar. Ver nota equivalente na SPEC 048 §3. O SQL abaixo é
> ilustrativo; a migração aplicada é [`059_conversation_channel.sql`](../supabase/migrations/059_conversation_channel.sql).
...
| 8   | ✅ `idx_conversations_last_customer_msg` (052)                                                          | Índice parcial sobre `(account_id, last_customer_message_at) WHERE status='open'`. Inalterado pela 059 (ex-057).                                                                                                                       | §7.1.4 |
...
A `session-window.ts` já declara no cabeçalho a razão de existir: ser _"o relógio da verdade
único"_ em vez de cada consumidor recalcular do seu jeito. A correção estende esse mesmo
...
o caso é diferente e a distinção importa: não é _desconhecimento_ do estado da janela — é a
...
### 7.1.4 Migração 059 (ex-057) com asserção

Seguindo o padrão de [041_assert_039_intact.sql](../supabase/migrations/041_assert_039_intact.sql)
e dos blocos `DO $$` da 052, a 059 termina verificando que não desfez a SPEC 045 — e **falha
alto** se desfez (os textos de exceção reais dizem `'059: ...'`, não `'057: ...'` — ver nota de
renumeração na SPEC 048 §3):
...
O editor exibe, ao selecionar esse gatilho: _"Este gatilho depende da janela de 24h e só vale
para canais WhatsApp Oficial. Sua conta não tem nenhum canal compatível."_
...
- no construtor de automações, sob o seletor de canal de desvio: _"Limite de segurança
...
`automation_logs` (`describeDenial()`), então o operador vê _"cold-send daily limit reached
(60/24h)"_ — a armadilha "não dispara, sem erro" continua fechada.
...
| **F5** | Migração 059 (ex-057); filtro e badge de canal no inbox; composer por capacidade; ficha de contato multicanal                                                                     | Contato que fala nos dois canais tem 2 threads e 1 ficha; filtro funciona; composer não oferece o que o canal não faz                                                                                             |
...
| **D-2 quebrar a janela de 24h da SPEC 045**                        | **Alta se ignorado** | **Crítico** | **§7.1 inteira**: um `resolveSessionWindow` ciente de canal, filtro de capacidade na varredura, âncora condicional, asserção na 059 (ex-057) e 5 critérios de aceite. Sem isso, ou o canal QR não envia nada, ou o Cloud reengaja errado |

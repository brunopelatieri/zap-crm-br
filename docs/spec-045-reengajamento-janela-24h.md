...
**Status:** 🟢 Implementada (5 fases, 2026-08-09) · **1ª revisão de arquitetura em 2026-08-09**: citações reverificadas; §5.3, §5.4, §5.5 e §5.7 retificadas; §11 adicionada · **2ª revisão em 2026-08-09 (Opus 5)**: um bug de corretude no despacho da varredura, dois de fidelidade de dados e quatro guardrails novos — sumário em §0, todos confirmados corrigidos no código em checkpoint pós-implementação (2026-08-10) · **Migrações 052 e 053 aplicadas em `vn` e `rs`** · §10 fechada em 2026-08-10 (pergunta 3 corrigida no código; 1, 2 e 5–8 já estavam decididas no código e ficam registradas)
...
> _hint_ visual no composer do inbox — o motor de automações é cego a ele.
...
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- | ------------ |
| 1   | **A varredura, como escrita em §5.5.2, dispara N² vezes.** `runAutomationsForTrigger()` executa **todas** as automações ativas daquele trigger na conta ([engine.ts:103-108](../src/lib/automations/engine.ts#L103-L108)); o loop de claim é _por automação_. Com duas automações do trigger na mesma conta, cada claim executa as duas → 4 execuções, 2 envios duplicados, e o claim não protege nada                                                                 | 🔴 Corretude — envio duplicado ao cliente | §5.5.3       |
...
| --- | ----------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| 1   | **Janela de sessão da Meta** (o assunto desta SPEC)                                       | Só como _hint_ de UI, client-side: [message-thread.tsx:228-257](../src/components/inbox/message-thread.tsx#L228-L257) | Se a próxima mensagem pode ser texto livre ou precisa ser template                     |
...
`sessionExpiredHint` (`messages/en.json:325` — _"24-hour session expired. Use
a template to re-engage."_). **Não existe coluna no banco** (`window_expires_at`
...
- `runAutomationsForTrigger()` ([:70](../src/lib/automations/engine.ts#L70)) tem exatamente **dois** call sites no
  ...
  `validateTriggerForActivation` ([:260](../src/lib/automations/validate.ts#L260)). _(Retificado nesta revisão: a redação
...
_lança_ quando não há conversa, e uma exceção dentro de
...
_(Seção da 2ª revisão. A pergunta "a guarda de §5.3 não deveria estar mais
fundo, no `meta-send`, para valer para todo mundo?" é legítima e merece
resposta escrita, não silêncio.)_
...
1. **Implementar o trigger genérico `time_based`** (agendamento tipo cron
   ...
   desta entrega. **Resolvido em
   [SPEC 046](spec-046-agendamento-visual.md)** (2026-08-10): construtor
   visual de agendamento + a varredura que finalmente faz este gatilho
   disparar de verdade.
...
um _range seek_ em vez de um scan. Isso só se sustenta se **toda** consulta
...
_(2ª revisão — sobre o predicado parcial.)_ `conversations.status` aceita
...
_(Seção da 2ª revisão. A decisão "escreve no webhook" de §5.2 está certa;
o que faltava era dizer **o quê** escrever. Copiar as linhas vizinhas
produz duas divergências silenciosas.)_
...
_(Retificação desta revisão — a primeira redação tratava os três step
types como equivalentes, e eles não são.)_
...
_(Seção da 2ª revisão. As três primeiras subseções de §5.3 tratam
`'skip' | 'fail' | 'fallback_template'` como um select de três opções.
As duas primeiras não fazem nada; a terceira muda de regime.)_
...
esquecer _(detalhadas nesta revisão)_:
...
_(Retificado nesta revisão: a redação anterior mostrava uma varredura
...
**uma** query por tick e vai embora.)_
...
_(Decisão desta revisão. A primeira redação propunha um endpoint novo.)_
...
arquivo instrui o operador a _preencher, rodar e descartar sem commitar_.
...
_(Retificação desta revisão. A proposta original — serializar a âncora da
janela dentro de `automation_logs.trigger_event` e checar existência antes
de disparar — tem dois defeitos, um de corretude e um de escala.)_
...
exatamente o cenário que o resto do repositório já resolve com claim: _"O
...
vezes"_ ([broadcasts/cron/route.ts:26-29](../src/app/api/broadcasts/cron/route.ts#L26-L29)); o mesmo padrão está em
...
_(Achado nº 1 da 2ª revisão. É um bug de composição: cada peça está
certa isolada, e juntas produzem envio duplicado — exatamente o que
§5.5.2 foi escrita para impedir.)_
...
_(Achado nº 4 da 2ª revisão. §5.5.1 decidiu **onde** a varredura roda e
acertou; falta dizer sob que teto de tempo ela roda.)_
...
_(Achado nº 8 da 2ª revisão. §5.5.2 define a criação da linha; nada é
dito sobre o que acontece depois dela.)_
...
**A escolha explícita.** Em mensageria, _at-most-once_ é o padrão certo:
...
2. **`contacts.opt_in_status != 'opted_out'`** (reusa `isOptedOut()` de
   ...
   _categoria ausente ou desconhecida como marketing_
...
_(Itens 6 a 9 acrescentados na 2ª revisão. Os cinco primeiros protegem o
...
por isso estão aqui e não "na implementação".)_
...
6. **Não reengajar com a bola do nosso lado.** 🔴 O caso: o cliente
   ...
   bot dispara _"Posso ajudar com mais alguma coisa?"_. Do lado de lá
...
_(2ª revisão — dois detalhes de UI que só aparecem lendo o arquivo:)_
...
_(Retificado nesta revisão — a redação anterior propunha `> 0`.)_
...
Também exigir inteiro (`Number.isInteger`). _(Retificado na 2ª revisão: a
...
de §2.3, não por simetria com um step que não a tem.)_
...
- Como argumentado em §8.1, este template é a principal peça pedagógica
  ...
  degrada _quality rating_, e sair no template pronto o transformaria no
  padrão de fato de toda conta nova.

### 5.9 Ganhos colaterais que a coluna e a função pagam sozinhas

_(Seção da 2ª revisão. Nada aqui é requisito da entrega; tudo aqui fica
barato **depois** da fase 1 e caro se for feito separado.)_
...
2. **D1 10:05** — agente responde. Não afeta a janela: só mensagem do
   cliente conta. _(2ª revisão: mas afeta a **elegibilidade** — é esta
   ...
   seria não reengajar, e sim aparecer na fila de quem deve resposta.)_
3. Cliente não escreve mais nada. Com margem de 4h, a conversa fica
   ...
   _(Retificado nesta revisão: a redação anterior dizia "às 20:00",
...
5. A automação manda `send_buttons`: _"Ainda precisa de ajuda com algo?
   Toque em uma opção 👇"_ com botões tipo "Sim, continuar" / "Não,
...
_(2ª revisão.)_ As duas primeiras linhas desta tabela descrevem consultas
...
antes de ele virar queda de _quality rating_, que é quando já não há o
...
| Risco                                                                                                                                                                          | Mitigação                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Política da Meta**: mensagens cujo único propósito é "manter a janela aberta", sem valor real ao usuário, alimentam reclamações e podem afetar a _quality rating_ do número. | Fora do controle técnico do CRM — registrar como orientação de produto/conteúdo: a automação deve oferecer algo de valor real (ex. "posso ajudar com mais alguma coisa?", não um botão vazio). Não é um bloqueio de implementação, é uma nota de responsabilidade a comunicar ao usuário do CRM na UI (tooltip/hint no builder). **Ver §8.1 para por que este risco é estruturalmente diferente do de um template.** |
...
_(Aprofundamento desta revisão. A linha da tabela acima descrevia o risco
em termos genéricos de "quality rating"; o que segue é o que torna esta
feature diferente de tudo que o CRM já manda.)_
...
| Onde o erro aparece                  | Na aprovação — antes de qualquer envio                                                                                                                       | **Depois do fato**, como reclamação do usuário e queda de _quality rating_ |
...
o _quality rating_ do número alimenta o tier de mensageria da Meta, que é
...
_(2ª revisão — o que mudou no plano.)_ Nenhuma fase nova: os achados de
...
_(Retificado nesta revisão.)_ As fases 1–2 **não consertam
...
> **Fechada em 2026-08-10.** As oito perguntas têm resposta — a maioria
> ficou decidida silenciosamente no código durante a implementação, sem
> nunca voltar para este documento; o checkpoint pós-implementação de
> 2026-08-10 conferiu cada uma contra o código atual e escreveu a
> decisão de volta aqui, riscada em vez de apagada, para que a escolha e
> o motivo dela não se percam. Só a pergunta 3 exigiu uma correção de
> código nesse checkpoint (ver a nota dela); as demais já estavam
> corretas, só não documentadas.

1. ~~Margem padrão para "janela fechando" — 4h é um chute razoável (dá
   tempo de o cliente ver e responder), mas é uma decisão de produto, não
   técnica.~~ **Resolvido no código: 4h (`DEFAULT_MARGIN_MINUTES = 240`
   em `window-trigger.ts`).** A mesma margem foi reaproveitada em SPEC
   046 §10, pergunta 3, para o `wait` do `follow_up_reminder` — em vez de
   inventar um segundo número para o mesmo raciocínio.
2. ~~O disparo de reengajamento deve respeitar o `send-window.ts` (horário
   ...
   ignorar o horário comercial?~~ **Resolvido no código: respeita.**
   Guardrail 9 em `window-scan.ts` —
   `isWithinSendWindow(now, DEFAULT_TIME_ZONE)` roda ANTES da fase A,
   então uma instância fora do horário permitido nem chega a consultar
   `automations`.
3. ~~Vale, já nesta entrega, também consertar o `follow_up_reminder`
   existente (trocar seu `wait 1 day` por uma lógica que respeite a
   janela), ou ele fica como está e só o novo template (§5.8) é
   oferecido?~~ **Resolvido em revisão pós-implementação (2026-08-10):
   consertado, não substituído.** `wait` reduzido de 1 dia para 20h —
   a mesma margem de 4h da pergunta 1 (`DEFAULT_MARGIN_MINUTES`) —, e
   `on_window_closed: 'skip'` explícito no `send_message` como rede de
   segurança. O template continua com uma limitação estrutural que
   `reengagement_before_window_closes` não tem (a espera é estática,
   não recalcula contra uma mensagem mais recente do cliente), mas
   deixa de mandar um free-form message exatamente no instante em que a
   janela fecha. Ver `templates.ts` e `templates.test.ts`.
4. ~~`last_customer_message_at` — popular via trigger de banco
   ...
   `AFTER INSERT` passa a valer o custo. _(2ª revisão: a decisão fica de
   ...
   Com a escrita filtrada de §5.2.1(a) o webhook empata; sem ela, perde.)_

**Perguntas novas da 2ª revisão:**

5. ~~A varredura deve pular conversa atribuída a um agente humano?~~
   (§5.6, item 8.) **Resolvido no código: pula.**
   `.is('assigned_agent_id', null)` na fase B de `window-scan.ts` —
   reengajar automaticamente uma conversa que já tem dono humano
   interfere no atendimento de alguém que pode estar prestes a responder.
6. ~~Reengajar conversa em `pending`, ou só `open`?~~ (§5.2.) **Resolvido
   no código: só `open`.** `.eq('status', 'open')` na fase B, o mesmo
   predicado literal do índice parcial `idx_conversations_last_customer_msg`
   (migração 052) — mudar para incluir `pending` exigiria recriar o
   índice, não só a query.
7. ~~O `fallback_template` deve respeitar opt-out?~~ (§5.3.4.) **Resolvido
   no código: sim.** `fallbackTemplateAllowed()` em `engine.ts` checa
   `isOptedOut` + `excludesOptedOut` pela categoria do template antes de
   mandar o fallback — a assimetria com o `send_template` manual (que
   continua sem checar nada) foi assumida, não fechada pelo item 2 de §4.
8. ~~A guarda de §5.3 deve mudar o `status` do log de `failed` para algo
   ...
   tela de logs deve mostrar. **Resolvido no código: o caminho barato.**
   `AutomationLogStatus` não ganhou um quarto valor — o rollup da
   automação fica `'success'`, e o `'skipped'` mora só no nível do step,
   em `AutomationLogStepResult` (`skipped()` em `engine.ts`). A
   automação inteira só vira `'failed'` quando um step lança de verdade;
   um `send_message` pulado por janela fechada não derruba o status geral.

## 11. Testes e rollout

_(Seção acrescentada nesta revisão — a SPEC 044 tem uma §10 equivalente e
o padrão do repositório é ter uma.)_
...
_(Casos acrescentados na 2ª revisão — cada um corresponde a um achado de
§0, e todos são de função pura ou de mock já existente:)_
...
_(3ª revisão — 2026-08-09. As duas versões anteriores desta seção
...
unidade por unidade, em vez de por fase inteira.)_

### 12.1 Por que "revise este diff" não basta aqui

Dos dez achados de §0, os três mais graves (nº 1, 2 e 4) têm a mesma
forma — _duas peças, cada uma correta, lidas em arquivos diferentes_:
...
Uma revisão que só olha o _diff_ da fase 4 nunca vê a Peça B desses três
...
`engine.ts` (ex. [:478-496](../src/lib/automations/engine.ts#L478-L496), o aviso de _defense in depth_ do
...
repetitivo). Nenhuma unidade precisa de Opus para _escrever_ — o padrão
